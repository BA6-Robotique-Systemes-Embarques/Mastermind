...
static void initializeThreads(void){
	...
	process_image_thd_start();//Image capture and analysis
	run_thd_start();//Main thread : responsible of motors and thread coordination
	display_thd_start();//Display of the hints on the red LEDs
}
...
int main(void){
	...
	motors_init();//Initializes the motors
    ...
    while (1) {
    	...
    	if(selector != old_selector && selector == SOLO_GAME){//Play with a random code generated by the robot
    		old_selector=selector;
    		setSoloMode(true);
    		starting_move();
    		initializeThreads();
